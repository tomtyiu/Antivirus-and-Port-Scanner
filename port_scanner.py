...
def port_check():
    ...
    for line in output:
        decoded_line = line.decode("utf-8")  # convert from byte format
        print (decoded_line)
...
def TCP_scan(ip,port):
    ...
    print("(address / port)",sock.getsockname())
    #print("Sock opt:", sock.getsockopt(socket.IPPROTO_IP, socket.SO_REUSEADDR, 1))
    if port == 22:
        print("SSH port")
    if port == 20:
        print("FTP port")
    if port == 80:
        print("HTTP port")
    if result == 0:
      print("TCP Port {}: \s Open".format(port))
      # print("Port is open", port)
    else:
       print("TCP Port {}: \s not Open".format(port))

def UDP_scan(ip, port):
    sock2= socket.socket(socket.AF_INET, socket.SOCK_DGRAM,socket.IPPROTO_IP)
    sock2.settimeout(1.0)
    sock2=socket.create_connection((ip,80))
    #print("(address / port)",sock2.getsockname())
    print(socket.getaddrinfo(ip,80, proto=socket.IPPROTO_UDP))
    #print("Sock opt:", sock2.getsockopt(socket.IPPROTO_IP, socket.SO_REUSEADDR, 1))
    result = sock2.connect_ex((ip,port))
    if port == 22:
        print("SSH port \n")
    if port == 20:
        print("FTP port \n")
...
if __name__ == '__main__':
    ...
    info('main line')
    port_sniff(0)
    remoteip=input("For Remote IP address: ")
    print("Start (0-65565) and__author__ = 'yiut'
# -*- coding: Non-UTF-8 -*-
# -----------------------------------------------------------------------------
# Name:        Port scanner and sniffer
# Purpose:	   Scans ports locally and sniff traffic
...
if __name__ == '__main__':
    ...
    port_sniff(0)
    remoteip=input("For Remote IP address: ")
    print("Start (0-65565) and end port (0-65565): ")
    try:
        start=int(input("Start port: "))
        if start<=0:
            start=int(input("Type start port again: "))
        end=int(input("End port: "))
        if end<start:
            end=int(input("Please type end port (end > start):"))
        elif end<=0:
            end=int(input("End port > 0: "))
    except ValueError:
